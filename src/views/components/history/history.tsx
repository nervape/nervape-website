...
export const TRANSFER_STATUS_STRING = {
    pending: 'In Progress',
    failed: 'Failed',
    committed: 'Completed'
};
...
function HistoryItem(props: {
    ...
    function formatTime(time: number) {
        ...
        return (
            <>
                <div>{`${year}/${month > 9 ? month : `0${month}`}/${
                    day > 9 ? day : `0${day}`
                }`}</div>
                <div>{`${hour > 9 ? hour : `0${hour}`}:${minute > 9 ? minute : `0${minute}`}:${
    ...
    return (
        ...
        >
            {loginWalletType !== LoginWalletType.UNIPASS_V3 && (
                <div className="index">{`#${history.tokenId}`}</div>
            )}
            <div className={`type ${showType()}`}>
                <div className="span">{showType()}</div>
            </div>
            <div className="date">{history.timestamp ? formatTime(history.timestamp) : '/'}</div>
            <div className={`status ${history.status.toLowerCase()}`}>
...
const History = (
    ...
    const { address, loginWalletType, setLoading, updateBalance, nftCoverImages } = props;
    ...
    useEffect(() => {
        if (!address || !loginWalletType) return;
        if (loginWalletType !== LoginWalletType.UNIPASS_V3) {
