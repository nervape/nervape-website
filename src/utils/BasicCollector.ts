...
export default class BasicCollector extends Collector {
    ...
    async collectCapacity(address: Address, neededAmount: Amount) {
        ...
        const minCapacity = new Amount('61', AmountUnit.ckb);
        ...
        for (const rawCell of rawCells) {
            ...
            if (typeScript === undefined || typeScript === null) {
                ...
                if (amountTotal.gte(neededAmount.add(minCapacity))) break;
            }
        }
        if (amountTotal.lt(neededAmount))
            throw new Error(
                `Could not find enough input capacity. Needed ${neededAmount.toString(
                    AmountUnit.ckb
                )}, found ${amountTotal.toString(AmountUnit.ckb)}.`
            );
    ...
    async collectSUDT(sudt: SUDT, address: Address, neededAmount: Amount) {
        ...
        for (const rawCell of rawCells) {
            ...
            if (amountSUDTTotal.gte(neededAmount)) break;
        }
        if (amountSUDTTotal.lt(neededAmount))
            throw new Error(
                `Could not find enough input SUDT cells. Needed ${neededAmount.toString(
                    0
                )}, found ${amountSUDTTotal.toString(0)}.`
            );
    ...
    async getBalance(address: Address) {
        ...
        const balance = cells.map(c => c.capacity).reduce((sum, cap) => (sum = sum.add(cap)));
    ...
    async getSUDTBalance(sudt: SUDT, address: Address) {
        ...
        for (const cell of cells) {
            if (
                !!cell.type &&
                cell.type.toHash() === sudtTypeHash &&
                cell.getHexData().length >= 34
            ) {
